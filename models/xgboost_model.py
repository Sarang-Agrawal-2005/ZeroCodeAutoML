...
def get_model_params_ui(problem_type):
    ...
    n_estimators = st.sidebar.slider("Number of Estimators", 50, 1000, 100, step=50)
    learning_rate = st.sidebar.slider("Learning Rate", 0.001, 0.5, 0.1, step=0.01)
    max_depth = st.sidebar.slider("Max Depth", 1, 20, 6)
    subsample = st.sidebar.slider("Subsample Ratio", 0.1, 1.0, 1.0, step=0.1)
    colsample_bytree = st.sidebar.slider("Column Subsample by Tree", 0.1, 1.0, 1.0, step=0.1)
